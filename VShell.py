...
class VShell:
    ...
    def command_not_found(*args):
        raise SyntaxError("No such a command")

    @staticmethod
    def assert_argument_number(args, *numbers):
    ...
    def pwd(self, *args):
        self.assert_argument_number(args, 0)

        print(self.current_path)

    def ls(self, *args):
        self.assert_argument_number(args, 0, 1)

        if args:
            path = self.add_to_path(args[0])
        else:
    ...
    def cd(self, *args):
        self.assert_argument_number(args, 1)
    ...
    def run(self):
        while True:
            ...
            try:
                self.commands.get(command[0], self.command_not_found)(*command[1:])
